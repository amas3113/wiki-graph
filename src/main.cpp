...
void printName(Graph* g){
    ...
    cout << "ID of article: ";
    cin >> a_id;
    if(a_id >= g->numToVertex.size()) {
        cout << "out of bounds. Enter smaller number" << "\n";
        printName(g);
    } else {
        cout << g->numToVertex[a_id].name << "\n";
    }
}
...
void runKosaraju(Graph& g){
    ...
    cout << "# of strongly connected components: " << components.size() << endl;
}
...
int main () {
    ...
    cout << "Please enter the vertices file:" << endl;
    cin >> input;
    g.parseVertices(input);

    cout << "Please enter the edges file:" << endl;
    cin >> input;
    g.parseEdges(input);

    cout << "Please enter the categories file:" << endl;
    cin >> input;
